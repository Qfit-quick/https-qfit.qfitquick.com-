...
export var CHALLENGE_ICONS = {
  ...
  // 팔이 몸통과 거의 겹쳐 보여서(2026-09-15 재점검) 봉 쪽으로 더 세운
  // 각도로 뻗고, 발끝 굽힘도 더 뚜렷하게 키웠다.
  row: figureProfile({
    head: [20, 24],
    torso: [[24, 28], [78, 56]],
    arm: [[24, 30], [16, 16], [12, 4]],
    leg: [[78, 56], [92, 50], [98, 62]],
    propsBefore: [barProp(10, 2, 26)]
  }),
  ...
  // 턱 홀드 계열 — 무릎을 가슴 쪽으로 완전히 당긴 지지 자세 (측면, 손은 바닥).
  // 원래 좌표는 사람 모양이 안 잡히는 갈고리 모양으로 나와서(2026-09-14
  // 재점검) leanTilt·plank 와 같은 "손 짚고 몸 앞으로" 비례를 가져오고
  // 다리만 접어 바닥선(groundProp)도 새로 붙였다.
  tuckHold: figureProfile({
    head: [32, 46],
    torso: [[38, 51], [58, 38]],
    arm: [[38, 53], [36, 70], [35, 88]],
    leg: [[58, 38], [64, 54], [46, 50]],
    propsBefore: [groundProp(92)]
  }),

  // 스트래들 계열 — 봉에 매달려 다리를 넓게 벌린 자세 (측면, 다리 2개).
  // 이 아이콘을 쓰는 운동 대부분(원레그·풀 프론트레버 등)이 실제로는
  // 매달리기인데 원래 좌표엔 봉도 없고 팔이 바닥 쪽으로 뻗어 있어 방향이
  // 거꾸로였다(2026-09-14 재점검) — tuckHang 처럼 팔을 위 봉으로 보내고
  // barProp 을 붙였다.
  straddleHold: figureProfile({
    head: [20, 26],
    torso: [[24, 28], [48, 34]],
    arm: [[24, 26], [20, 14], [18, 4]],
    leg: [[48, 34], [70, 18], [92, 6]],
    leg2: [[48, 34], [70, 50], [92, 66]],
    propsBefore: [barProp(4, 2, 36)]
  }),
  ...
  // backLeverTuck 과 거의 같은 모양으로 나와서(2026-09-15 재점검) 둘 다
  // 새로 그렸다 — 이쪽은 몸을 위로 접어 봉 가까이 뭉친 모습으로.
  tuckHang: figureProfile({
    head: [18, 26],
    torso: [[22, 30], [40, 46]],
    arm: [[22, 28], [18, 14], [16, 4]],
    leg: [[40, 46], [50, 34], [34, 32]],
    propsBefore: [barProp(4, 2, 32)]
  }),
  ...
  // 백레버 턱 — 봉에 매달려 몸의 앞면이 바닥을 향하도록 아치형으로 버티는
  // 자세(턱 프론트레버와 반대 방향). tuckHang 과 거의 같은 모양으로 나와서
  // (2026-09-15 재점검) 이쪽은 몸이 봉 아래로 더 길게 늘어진 모습으로
  // 다시 그려 구분했다.
  backLeverTuck: figureProfile({
    head: [30, 52],
    torso: [[26, 46], [22, 26]],
    arm: [[22, 24], [18, 14], [16, 4]],
    leg: [[26, 46], [44, 52], [40, 64]],
    propsBefore: [barProp(4, 2, 32)]
  }),
  ...
  // 피스톨 스쿼트 트랙(2026-09-24) 5개 추가 — 이 트랙만 기존 7개와 달리
  // 하체 위주라 다리 굽힘 각도가 핵심이다.

  // 맨몸 스쿼트 — 서서 두 다리를 같이 굽히는 기본 스쿼트 (측면, 두 다리가
  // 겹쳐 보이므로 plank·pike 처럼 다리 하나로 실루엣만 잡는다)
  squat: figureProfile({
    head: [42, 16],
    torso: [[42, 22], [44, 48]],
    arm: [[42, 24], [24, 28], [10, 30]],
    leg: [[44, 48], [28, 54], [42, 88]],
    propsBefore: [groundProp(92)]
  }),

  // 피스톨 스쿼트 — 한쪽 다리는 앞으로 곧게 뻗고 반대쪽만 깊이 굽히는
  // 싱글레그 스쿼트. 이 트랙의 대표 자세라 서포트·박스·네거티브 등
  // 여러 변형이 이 아이콘을 같이 쓴다(다른 트랙의 'pull'이 여러 턱걸이
  // 변형을 겸하는 것과 같은 방식).
  pistolSquat: figureProfile({
    head: [36, 28],
    torso: [[38, 33], [42, 56]],
    arm: [[38, 35], [20, 40], [6, 44]],
    leg: [[42, 56], [26, 62], [40, 90]],
    leg2: [[42, 56], [68, 50], [94, 48]],
    propsBefore: [groundProp(92)]
  }),

  // 서포트 잡고 하는 피스톨 스쿼트 — pistolSquat 과 같은 자세에 문틀·
  // 기둥을 잡는 손 옆 세로 소품만 더한다
  pistolAssist: figureProfile({
    head: [36, 28],
    torso: [[38, 33], [42, 56]],
    arm: [[38, 35], [20, 40], [6, 44]],
    leg: [[42, 56], [26, 62], [40, 90]],
    leg2: [[42, 56], [68, 50], [94, 48]],
    propsBefore: [groundProp(92)],
    propsAfter: ['<line x1="4" y1="6" x2="4" y2="52" stroke="var(--accent)" stroke-width="6" stroke-linecap="round" opacity="0.9"/>']
  }),

  // 불가리안 스플릿 스쿼트 등 스태거 스탠스 — 앞다리는 굽혀 딛고 뒷다리는
  // 뒤로(살짝 들려) 뻗는 자세. lungeStretch(정적 스트레칭용)와 다리
  // 배치가 비슷해 보이지만 뒷다리를 더 들어 올려 '딛고 미는' 힘 자세로
  // 구분했다.
  splitSquatStep: figureProfile({
    head: [28, 20],
    torso: [[30, 26], [38, 52]],
    arm: [[30, 28], [18, 36], [8, 42]],
    leg: [[38, 52], [28, 66], [42, 90]],
    leg2: [[38, 52], [60, 48], [84, 44]],
    propsBefore: [groundProp(92)]
  }),

  // 힙 브릿지 — 바닥에 누워 어깨는 대고 엉덩이만 들어올리는 자세 (측면).
  // core(할로우바디)와 눕는 방향은 같지만 무릎을 굽혀 발을 바닥에 딛고
  // 엉덩이 쪽을 살짝 더 들어 '브릿지' 임을 구분했다.
  hipBridge: figureProfile({
    head: [8, 70],
    torso: [[14, 70], [56, 62]],
    arm: [[14, 71], [9, 80], [5, 88]],
    leg: [[56, 62], [64, 80], [58, 92]],
    propsBefore: [groundProp(94)]
  })
};
