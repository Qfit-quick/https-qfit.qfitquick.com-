...
// maybeAutoEnable() 이 처음 접속했을 때(한 번만) 그 팝업을 대신 띄워 준다
// (app.js 부팅 시퀀스, loadProfile() 직후에서 호출).
//
// 처음 접속 시로 정한 것은 명시적 요청(2026-09-16)이다 — 앱이 뭘 하는지도
// 모르는 시점에 물으면 거절 확률이 높고, 그 거절은 브라우저가 기억해서
// 코드로는 되돌릴 수 없다는 점은 그대로 유효하니 참고. (처음엔 "첫 완주
// 직후"로 그 위험을 피하려 했으나, 사용자가 "처음 접속 시"로 바꿔 달라고
// 다시 요청해 이렇게 바뀌었다.)
//
 ...
 *  물으면 그 자체가 성가신 알림이 된다. 처음 접속했을 때 한 번 부르는
 *  것을 전제로 한다(app.js 부팅 시퀀스, loadProfile() 직후). */
