...
import { KNOWLEDGE, pick } from '../../src/chat/knowledge.js';

test('KNOWLEDGE 가 비어있지 않고 필수 필드를 갖는다(title/body 는 {ko,en,zh})', () => {
  ...
  for (const entry of KNOWLEDGE.slice(0, 20)) {
    ...
    for (const l of ['ko', 'en', 'zh']) {
      assert.equal(typeof entry.title[l], 'string');
      assert.ok(entry.title[l].length > 0, `title.${l} 빈 항목: ${entry.id}`);
    }
  }
});
...
test('retrieve: 운동 이름으로 정확히 찾는다', () => {
  ...
  assert.ok(pick(hits[0].title, 'ko').includes('푸쉬업'));
});

test('retrieve: 영어로 물어도 찾는다(title.en 도 본다)', () => {
  const hits = retrieve('how to do a Squat');
  assert.ok(hits.some((h) => pick(h.title, 'ko') === '스쿼트'));
});
